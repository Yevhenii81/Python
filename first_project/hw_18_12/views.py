...
# ===== КНИГИ =====
@login_required
@permission_required('hw_18_12.view_book', raise_exception=True)
...
@login_required
@permission_required('hw_18_12.add_book', raise_exception=True)
...
@login_required
@permission_required('hw_18_12.change_book', raise_exception=True)
...
@login_required
@permission_required('hw_18_12.delete_book', raise_exception=True)
def book_delete(request, book_id):
    book = get_object_or_404(Book, id=book_id)
    if request.method == 'POST':
        book.delete()
        return redirect('book_list')
    return render(request, 'hw_18_12/confirm_delete.html', {'object': book})


# ===== ЧИТАТЕЛИ =====
@login_required
@permission_required('hw_18_12.view_reader', raise_exception=True)
...
@login_required
@permission_required('hw_18_12.add_reader', raise_exception=True)
...
@login_required
@permission_required('hw_18_12.change_reader', raise_exception=True)
...
@login_required
@permission_required('hw_18_12.delete_reader', raise_exception=True)
def reader_delete(request, reader_id):
    reader = get_object_or_404(Reader, id=reader_id)
    if request.method == 'POST':
        reader.delete()
        return redirect('reader_list')
    return render(request, 'hw_18_12/confirm_delete.html', {'object': reader})
